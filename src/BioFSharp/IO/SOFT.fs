...
open BioFSharp.FileFormats.SOFT
open System.IO
open FSharpAux.IO

module SOFT =

    /// SOFT specification tokens generated by 'GenerateSOFTTypes.fsx' from the specification documents
    ...
    module internal Tokenization =
        ...
        //Regarding to SOFT specifications:
    ...
    module internal Parsing =
        ...
        let internal listMustContainExactlyOne failmessage l =
            match List.tryExactlyOne l with
            | Some v -> v
            | _ -> failwith failmessage

        let internal listMustContainOneOrMore failmessage (l: 'a list) =
            if l.Length >= 1 then
                l
            else failwith failmessage

        let internal createSampleRecord accession (specList:SOFTSampleSpecifications list) =
            {
                Accession = accession
        ...
        let internal createPlatformRecord accession (specList: SOFTPlatformSpecifications list)=
            {
                Accession = accession
                Title =
                    specList
        ...
        let internal createSeriesRecord accession (specList : SOFTSeriesSpecifications list) =
            {
